...
app.post('/buscar', async (req, res, next) => {
    ...
    if (!startDate || !endDate) {
        return res.status(400).send({ "message": "Check parameters", "startDate": startDate === undefined ? null : startDate, "endDate": endDate === undefined ? null : endDate });
    }

    const data = await startCrawler();
    res.status(200).send({ "message": "Success", "startDate": startDate, "endDate": endDate , response : data });
});

app.listen(3000);


startCrawler = async function () {
